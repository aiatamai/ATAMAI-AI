<script lang="ts">
	...
	import { ATAMAI_NAME, config, user, socket } from '$lib/stores';
			...
			<div class="w-full px-10 min-h-screen flex flex-col text-center">
				{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
					<div class=" my-auto pb-10 w-full sm:max-w-md">
						...
						>
							<div>
								{$i18n.t('Signing in to {{ATAMAI_NAME}}', { ATAMAI_NAME: $ATAMAI_NAME })}
							</div>
							...
							>
								<div class="mb-1">
									<div class=" text-2xl font-medium">
										{#if $config?.onboarding ?? false}
											{$i18n.t(`Get started with {{ATAMAI_NAME}}`, { ATAMAI_NAME: $ATAMAI_NAME })}
										{:else if mode === 'ldap'}
											{$i18n.t(`Sign in to {{ATAMAI_NAME}} with LDAP`, { ATAMAI_NAME: $ATAMAI_NAME })}
										{:else if mode === 'signin'}
											{$i18n.t(`Sign in to {{ATAMAI_NAME}}`, { ATAMAI_NAME: $ATAMAI_NAME })}
										{:else}
											{$i18n.t(`Sign up to {{ATAMAI_NAME}}`, { ATAMAI_NAME: $ATAMAI_NAME })}
										{/if}
									</div>
										...
										{#if mode === 'signup'}
											<div class="mb-2">
												...
												<input
													...
													type="text"
													id="name"
													class="my-0.5 w-full text-sm outline-none bg-gray-100/50 dark:bg-gray-800/50 border border-gray-300 dark:border-gray-700 rounded-xl px-4 py-2.5 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:border-accent-600 focus:ring-2 focus:ring-accent-600/20 transition-all"
													autocomplete="name"
								...
								<div class="mt-5">
									{#if $config?.features.enable_login_form || $config?.features.enable_ldap || form}
										{#if mode === 'ldap'}
											<button
												class="bg-gradient-to-r from-accent-600 to-accent-700 hover:from-accent-700 hover:to-accent-800 text-white shadow-lg hover:shadow-glow transition-all w-full rounded-full font-medium text-sm py-3"
												type="submit"
											>
												{$i18n.t('Authenticate')}
											</button>
										{:else}
											...
											{#if $config?.features.enable_signup && !($config?.onboarding ?? false)}
												<div class=" mt-4 text-sm text-center">
													...
													<button
														class="font-medium underline text-accent-600 hover:text-accent-700 transition-colors"
														type="button"
														on:click={() => {
															if (mode === 'signin') {
																mode = 'signup';
